...
function fieldByCodeLocal(code){return fieldByCode(props.fields,code)}
  function isContinuousField(field){return String(field?.valueSetMatchMode||field?.valueSetBinding?.matchMode||'').toUpperCase()==='CONTINUOUS'}
  function operatorOptions(condition){const field=fieldByCodeLocal(condition.fieldCode);const kind=field?.kind;const options=[];if(isContinuousField(field)) options.push({value:'BETWEEN',label:'位于连续范围'});else if(kind==='DATETIME') options.push({value:'BETWEEN',label:'使用试算周期'});if(kind==='VALUE_SET' || (kind==='CODE' && field?.options?.length)) options.push({value:'IN_VALUE_SET',label:'属于标准值'});return options}
  function resetConditionValue(condition){const field=fieldByCodeLocal(condition.fieldCode);condition.parameter=condition.operator==='BETWEEN'&&!isContinuousField(field)?'period':undefined;condition.value=isContinuousField(field)&&condition.operator==='BETWEEN'?{start:'',end:''}:'';condition.itemCodes=[]}
  function fieldChanged(condition,value){condition.fieldCode=value||'';condition.operator=operatorOptions(condition)[0]?.value||'';resetConditionValue(condition);emitUpdate()}
  function operatorChanged(condition,value){condition.operator=value||'';resetConditionValue(condition);emitUpdate()}

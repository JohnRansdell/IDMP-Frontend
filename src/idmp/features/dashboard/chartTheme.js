...
export const DASHBOARD_PALETTE = ['#4f8583', '#7797b1', '#baa16d', '#8f9a80', '#ab8686', '#8795a5']
export function createDashboardChartTheme(option = {}) {
  ...
  return {
    ...option, color: DASHBOARD_PALETTE, backgroundColor: 'transparent',
    ...
    series: (Array.isArray(option.series) ? option.series : option.series ? [option.series] : []).map(series => ({
      ...series,
      ...(series.type === 'line' ? { showSymbol: false, lineStyle: { ...series.lineStyle, width: 2.5 } } : {}),
      ...(series.type === 'bar' ? { itemStyle: { ...series.itemStyle, borderRadius: [3, 3, 0, 0] } } : {}),
      ...(series.type === 'pie' ? { radius: ['48%', '70%'], itemStyle: { ...series.itemStyle, borderWidth: 2, borderColor: '#fff' } } : {})
    }))
  }
}

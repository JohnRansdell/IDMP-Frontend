...
function operatorOptions(condition){const kind=fieldByCodeLocal(condition.fieldCode)?.kind;return kind==='DATETIME'?[{value:'BETWEEN',label:'在试算统计周期内'}]:[]}
function fieldChanged(condition,value){condition.fieldCode=value||'';condition.operator=operatorOptions(condition)[0]?.value||'';condition.parameter=condition.operator==='BETWEEN'?'period':undefined;condition.value='';emitUpdate()}
function operatorChanged(condition,value){condition.operator=value||'';condition.parameter=condition.operator==='BETWEEN'?'period':undefined;condition.value='';emitUpdate()}
function valueChanged(condition,value){condition.value=value;emitUpdate()}
function emitUpdate(){emit('update')} function newPredicate(){return{id:`p-${Date.now()}-${Math.random()}`,nodeType:'PREDICATE',fieldCode:'',operator:'',value:''}} function addPredicate(){props.node.children.push(newPredicate());emitUpdate()} function addGroup(){props.node.children.push({id:`g-${Date.now()}`,nodeType:'AND',children:[newPredicate()]});emitUpdate()} function addNot(){props.node.children.push({id:`n-${Date.now()}`,nodeType:'NOT',child:{id:`g-${Date.now()}`,nodeType:'AND',children:[newPredicate()]}});emitUpdate()} function removeChild(index){props.node.children.splice(index,1);emitUpdate()} function dragStart(index){movingIndex=index} function drop(index){if(movingIndex<0||movingIndex===index)return;const [item]=props.node.children.splice(movingIndex,1);props.node.children.splice(index,0,item);movingIndex=-1;emitUpdate()}
</script>

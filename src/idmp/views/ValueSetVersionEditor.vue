...
async function load() { loading.value = true; error.value = ''; try { const [detail, itemRows] = await Promise.all([fetchValueSetVersion(versionId), fetchValueSetItems(versionId)]); valueSet.value = detail?.valueSet || null; version.value = detail?.version || detail; items.value = copyItems(itemRows); originalItems.value = copyItems(itemRows); validation.value = null } catch (err) { error.value = err?.message || '值集版本加载失败。' } finally { loading.value = false } }

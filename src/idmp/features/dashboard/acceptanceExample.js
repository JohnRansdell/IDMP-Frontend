...
export function createAcceptanceExampleSchema({ id = 'quality-overview', sceneCode = '' } = {}) {
  return normalizeDashboardSchema({ version: 1, id, name: '质量安全综合看板', dashboardType: 'scene', sceneCode, category: '质量安全', scope: 'hospital', layout: { engine: 'gridstack', columns: 24, float: false }, appearance: { background: { type: 'gradient', value: 'linear-gradient(135deg,#eaf5ff 0%,#f7fbff 48%,#eef9f6 100%)', intensity: 100 } }, globalFilters: createQualitySafetyGlobalFilters(), widgets: [
    widget('acceptance-kpi', '质量安全核心指标', 'kpi', { x: 0, y: 0, w: 4, h: 6 }, { style: { background: '#ffffff', borderColor: '#b9d9ef', borderWidth: 1, borderStyle: 'solid', borderRadius: 8, shadow: 'sm', padding: 0, opacity: 1 }, dataBinding: binding([], ['indicatorValue']) }),
    widget('acceptance-bar', '科室质量对比', 'bar', { x: 4, y: 0, w: 10, h: 6 }, { style: { background: '#ffffff', borderColor: '#b9d9ef', borderWidth: 1, borderStyle: 'solid', borderRadius: 8, shadow: 'sm', padding: 0, opacity: 1 }, dataBinding: binding(['department'], ['indicatorValue'], [], [{ field: 'month', operator: 'equals', value: '2026-09' }]), interaction: { clickAction: 'cross-filter', clickFilter: { enabled: true, field: 'department', targetWidgetIds: ['acceptance-table'] } } }),
    widget('acceptance-drill', '组织层级分析', 'bar', { x: 14, y: 0, w: 10, h: 6 }, { style: { background: '#ffffff', borderColor: '#b9d9ef', borderWidth: 1, borderStyle: 'solid', borderRadius: 8, shadow: 'sm', padding: 0, opacity: 1 }, dataBinding: { ...binding(['department'], ['value']), dataset: 'acceptance-drill' }, interaction: { clickAction: 'drill', drill: { hierarchy: ['department', 'medicalGroup', 'doctor'] } } }, 'UAT_ORGANIZATION_DRILL', '[UAT] 组织层级下钻数据'),
    widget('acceptance-line', '月度趋势', 'line', { x: 0, y: 6, w: 13, h: 7 }, { style: { background: '#ffffff', borderColor: '#b9d9ef', borderWidth: 1, borderStyle: 'solid', borderRadius: 8, shadow: 'sm', padding: 0, opacity: 1 }, dataBinding: binding(['month'], ['indicatorValue']) }),
    widget('acceptance-table', '质量明细', 'table', { x: 13, y: 6, w: 11, h: 7 }, { style: { background: '#ffffff', borderColor: '#b9d9ef', borderWidth: 1, borderStyle: 'solid', borderRadius: 8, shadow: 'sm', padding: 0, opacity: 1 }, tableColumns: ['month', 'department', 'disease', 'indicatorValue'], dataBinding: binding(['department'], ['indicatorValue'], [], [{ field: 'month', operator: 'equals', value: '2026-09' }]) })
  ] })
}

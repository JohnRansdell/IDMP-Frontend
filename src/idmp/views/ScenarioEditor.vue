<template>
  <div class="idmp-page scenario-editor">
    <PageHeader :title="form.name || '场景编辑'" :status="form.publicationStatus" :status-label="publicationStatusLabel(form.publicationStatus)">
      ...
      <template #actions><el-button @click="router.push({ name: 'ScenarioList' })">返回目录</el-button><el-button type="primary" :loading="saving" :disabled="!editable" @click="saveAll">保存草稿</el-button><el-button :loading="validating" :disabled="!editable" @click="validate">校验</el-button><el-button type="success" :loading="publishing" :disabled="!editable || !validationValid" @click="publish">发布</el-button></template>
    </PageHeader>
    <el-alert v-if="!editable" title="当前为已发布版本，只读查看。请从场景目录创建新版本后编辑。" type="info" show-icon :closable="false" class="mb16" />
    ...
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="校验诊断" name="validation"><el-empty v-if="!diagnostics.length" description="尚未产生诊断信息" /><el-alert v-for="(item, index) in diagnostics" :key="index" :title="item.message || item.code || '校验问题'" :type="item.level === 'ERROR' ? 'error' : 'warning'" show-icon :closable="false" class="diagnostic" /></el-tab-pane>
    </el-tabs>
  </div>
</template>
...
import { fetchScenarioMergedPreview, fetchScenarioVersion, fetchScenarioVersions, publishScenarioVersion, replaceScenarioIndicators, replaceScenarioOverrides, updateScenarioVersion, validateScenarioVersion } from '@/idmp/api/modules/scenarios'
import { normalizePage, OVERRIDE_TYPES, PERIOD_TYPES, publicationStatusLabel, SCENARIO_TYPES, scenarioDetailToForm, scenarioFormToPatch, toIndicatorBinding, toOverridePayload } from '@/idmp/api/adapters/scenario'
...
const form = reactive({ scenarioId: '', versionId: '', code: '', name: '', type: 'CUSTOM', description: '', governingOrgName: '', defaultPeriodType: 'MONTHLY', defaultParameters: {}, defaultExclusionDsl: { nodeType: 'TRUE' }, defaultExclusionDisplayText: '', defaultDataSourcePriority: [], displayText: '', effectiveStartDate: '', effectiveEndDate: '', effectiveRange: [], resourceVersion: 0, publicationStatus: 'DRAFT', versionNo: '-', indicators: [], overrides: [], defaultExclusionDslText: '{\n  "nodeType": "TRUE"\n}' })
const editable = computed(() => form.publicationStatus === 'DRAFT')
const selectedIndicatorIds = computed({ get: () => form.indicators.map((item) => Number(item.indicatorVersionId)), set: (ids) => { const old = new Map(form.indicators.map((item) => [Number(item.indicatorVersionId), item])); form.indicators = ids.map((id, index) => old.get(Number(id)) || toIndicatorBinding(indicatorOptions.value.find((item) => Number(item.id || item.indicatorVersionId || item.versionId) === Number(id)) || { indicatorVersionId: id }, index)); form.indicators.forEach((item, index) => { item.displayOrder = index }) } })
const boundIndicators = computed(() => form.indicators.map((binding) => {
  const option = indicatorOptions.value.find((item) => Number(item.id || item.indicatorVersionId || item.versionId) === Number(binding.indicatorVersionId))
  if (option) {
...
async function validate() { validating.value = true; try { const result = await validateScenarioVersion(form.versionId); diagnostics.value = result?.diagnostics || []; validationValid.value = result?.valid === true; form.resourceVersion = result?.resourceVersion ?? form.resourceVersion; validationMessage.value = validationValid.value ? '校验通过，可以发布。' : '校验未通过，请处理诊断信息。'; activeTab.value = 'validation' } catch (error) { await handleError(error) } finally { validating.value = false } }
async function publish() { publishing.value = true; try { const result = await publishScenarioVersion(form.versionId, form.resourceVersion, `scenario-publish-${form.versionId}-${form.resourceVersion}`); applyDetail(result); ElMessage.success('场景版本已发布') } catch (error) { await handleError(error) } finally { publishing.value = false } }

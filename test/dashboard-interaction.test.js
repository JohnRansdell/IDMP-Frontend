...
test('persisted dependency and interaction configuration rejects runtime or malformed data', () => {
  ...
  assert.deepEqual(validateWidgetInteraction({ clickAction: 'drill', drill: { hierarchy: ['department', 'medicalGroup', 'doctor'] } }), [])
  assert.ok(validateWidgetInteraction({ clickFilter: { enabled: true, field: 'department', targetWidgetIds: ['target', 'target'] } }).length)
  assert.ok(validateWidgetInteraction({ clickAction: 'drill', drill: { hierarchy: ['department', 'department'] } }).length)
